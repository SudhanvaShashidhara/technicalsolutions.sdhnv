...
export const handle: Handle = async ({ event, resolve }) => {
	...
	const ogt_landing_page_url =
		event.url.pathname.includes('ogt/landing-page') && event.url.searchParams.get('gclid');
	if (ogt_landing_page_url) {
		...
		if (current_gclid !== new_gclid) {
			...
			const new_url =
				event.url.href.split('?')[0] + '?' + search_params.toString() + event.url.hash;
